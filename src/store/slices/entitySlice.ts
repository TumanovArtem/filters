...
export const fetchEntityData = createAsyncThunk('entity/fetch', async () => {
    const response = await Promise.all([
        ...
        axios
            ...
            .then(addType(EntityTypeEnum.STARSHIP)),
    ]);
...
export const entitySlice = createSlice({
    ...
    reducers: {
        filterEntity: (
            state,
            { payload: entityFilter }: PayloadAction<EntityFilterType>
        ) => {
            state.filteredData = state.data
                .filter((entity) => entity.name.includes(entityFilter.name))
                .filter((entity) =>
                    entityFilter.type.length
                        ? entityFilter.type.includes(entity.type)
                        : true
                );

            if (
                entityFilter.type.length === 1 &&
                entityFilter.type[0] === EntityTypeEnum.PERSON
            ) {
                if (entityFilter.personFilter?.hair_color) {
                    state.filteredData = (
                        state.filteredData as Person[]
                    ).filter(
                        (person) =>
                            person.hair_color ===
                            entityFilter.personFilter.hair_color
                    );
                }
                if (entityFilter.personFilter?.eye_color) {
                    state.filteredData = (
                        ...
                        (person) =>
                            person.eye_color ===
                            entityFilter.personFilter.eye_color
                    );
                }
            }
        },
...
export const selectEntityFilteredData = (state: RootState) =>
    state.entity.filteredData;
